...
const Reviews = ({ product }) => {
    ...
    return (
        <div className='my-4'>
            ...
            <div className='my-10'>
                {
                    product?.reviewDetails?.length === 0 ? emptyReview : <>
                        <div>
                            {
                                product?.reviewDetails.map((review, idx) => <>
                                    <div key={idx}>
                                        <div className='flex flex-col items-start justify-center gap-1 text-gray-900 text-lg'>
                                            ...
                                            <h2 className='text-sm text-gray-700'>By <span className='text-blue-700 font-medium'>{review?.customer}</span><p className='inline'> on </p><span className=''>{new Date(review?.postedAt).toDateString()}</span></h2>
                                        </div>
                                    </div>
                                </>)
                            }
