...
const MyOrders = () => {
    ...
    const {data: myOrders, refetch} = useQuery({
        queryKey: 'myOrders',
        queryFn: ()=>{
            const orders = axiosPublic.get(`/orders?buyer=${user?.email}`)
                                .then(res=> res.data)
            return orders;
        }
    })
    return (
        <div className='px-12'>
            <h2 className='my-4 text-xl font-open font-semibold'>Order History</h2>
                                ...
                                <div class="overflow-hidden border border-gray-200 md:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            {
                                                myOrders?.map(order=><MyOrderRow order={order} />)
                                            }
                                        </tbody>
                                    </table>
                                </div>
