...
const SellerOrders = () => {
    ...
    const {data: allOrders, refetch} = useQuery({
        queryKey: 'allOrders',
        queryFn: ()=>{
            const orders = axiosPublic.get(`/orders?seller=${user?.email}`)
                                .then(res=> res.data)
            return orders;
        }
    });
    ...
    return (
        <div className='px-12'>
            <h2 className='my-4 text-xl font-open font-semibold'>All Orders</h2>
                                ...
                                <div class="overflow-hidden border border-gray-200 md:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            {
                                                allOrders?.map(order=><SellerOrderRow order={order} refetchOrder={refetch} />)
                                            }
                                        </tbody>
                                    </table>
                                </div>
